...
class _SupportProjectState extends State<SupportProject> {
  final svg = SvgPicture.asset("assets/invest.svg", height: 300,);
                    ...
                    onPressed: () async{
                      if(supportPageForm.currentState.validate())
                        {   
                            ...
                            final x = await FirebaseFirestore.instance.collection("campaigns").doc(widget.campaignModel.id.toString()).collection("notApproved").doc(currentUser.address).get();
                            final y = await FirebaseFirestore.instance.collection("campaigns").doc(widget.campaignModel.id.toString()).collection("investments").doc(currentUser.address).get();
                            if(widget.campaignModel.ownedByInvestorTotal + (AmountToBeInvested/widget.campaignModel.totalAmount)*100 >100){
                              Fluttertoast.showToast(
                                msg: "You investment amount exceed the requirements of fundRaiser.",
                                backgroundColor: Colors.red[800],
                                textColor: Colors.white,
                                gravity: ToastGravity.CENTER);
                            }
                            else if(currentUser.address==widget.campaignModel.publisherAddress){
                              Fluttertoast.showToast(
                            ...
                            else if(currentUser.balance<AmountToBeInvested){
                              Fluttertoast.showToast(
                                msg: "Balance too low",
                                backgroundColor: Colors.red[800],
                                textColor: Colors.white,
                              ...
                              await FirebaseAdd().addInterestApproval(investor: currentUser,interest: Interest, investAmount: AmountToBeInvested, campaign: widget.campaignModel).then((value){
                                setState(() {
                                  loading=false;
                                });
                                Fluttertoast.showToast(
